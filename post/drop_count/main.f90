program drop_count
  ...
  real(8), allocatable :: drop_tsum(:)   ! accumulator for temperature
  ...
  allocate( stack_x(nx*ny), stack_y(nx*ny) )
  ...
  do snap = t_start, t_end, t_step
    ...
    do j = 1, ny
      do i = 1, nx
        ...
        if ( mask(i,j) .and. .not. visited(i,j) ) then
          ...
          drop_xmin(n_drops) = i;  drop_xmax(n_drops) = i
          drop_ymin(n_drops) = j;  drop_ymax(n_drops) = j
          ...
          do while (stack_top > 0)
            ...
            ! update bounding box
            drop_xmin(n_drops) = min(drop_xmin(n_drops), ci)
            drop_xmax(n_drops) = max(drop_xmax(n_drops), ci)
            drop_ymin(n_drops) = min(drop_ymin(n_drops), cj)
            drop_ymax(n_drops) = max(drop_ymax(n_drops), cj)

            ! visit 8 neighbours
            do d = 1, n_nb
              ni = mod(ci + dx(d) - 1 + nx, nx) + 1   ! periodic x
              nj = cj + dy(d)
    ...
    do d = 1, n_drops
      write(20,'(I6, 2X, I8, 4(2X,I6), 2X, ES15.7)') d, &
        drop_area(d),                          &
